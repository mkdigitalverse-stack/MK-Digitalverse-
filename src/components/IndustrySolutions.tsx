...
export const IndustrySolutions: React.FC<IndustryProps> = ({
  ...
  return (
    <section id="industries" className="py-20 md:py-28 bg-[#050505] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-display font-extrabold text-white tracking-tight">
            Tailored Growth Systems for Specialized Verticals
          </h2>
          <p className="mt-4 text-base sm:text-lg text-zinc-300">
            We don't do generalist work. Our systems are built around the unit economics, patient psychology, and high-value booking workflows of Healthcare and Luxury Venues in India.
          </p>

          {/* Industry Tab Buttons */}
          <div className="mt-8 inline-flex p-1.5 rounded-2xl bg-zinc-950 border border-white/10">
            <button
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-12 max-w-4xl mx-auto">
          <span className="text-xs font-semibold uppercase text-zinc-500 mr-2">Target Segments:</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {data.verticals.map((vert) => (
            <div 
              key={vert.id}
              className="glass-panel glass-panel-hover rounded-2xl p-6 border border-white/10 flex flex-col justify-between bg-[#09090b]"
            >
              <div>
        ...
        <div className="glass-panel rounded-2xl p-8 border border-white/10 mb-16 bg-[#09090b]">
          <div className="mb-8">
            <h3 className="text-2xl font-bold text-white">
              The 4 Core Pillars of {activeIndustry === 'healthcare' ? 'Healthcare Growth System™' : 'Venue Growth System™'}
            </h3>
            <p className="text-sm text-zinc-400 mt-1">
              Every layer is integrated to maximize patient/couple conversion and operational efficiency.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {data.growthPillars.map((pillar, idx) => (
              <div key={idx} className="flex gap-4 items-start p-4 rounded-xl bg-zinc-900/60 border border-white/5 hover:border-white/15 transition-all">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {data.metrics.map((m, idx) => (
            <div key={idx} className="p-6 rounded-2xl bg-zinc-950 border border-white/10 text-center">
              ...
              <div className="text-xs text-zinc-400 mt-1">{m.subtext}</div>
            </div>
          ))}
        </div>
